...
final class ClawTests: XCTestCase {
    ...
    func testMissionRunSummaryDerivesReviewPriorityQueue() throws {
        ...
        let idleReviewRadar = idleStore.missionRunSummary.macAgentReviewRadar
        XCTAssertFalse(idleReviewRadar.isReviewable)
        XCTAssertEqual(idleReviewRadar.totalCount, 0)
        XCTAssertEqual(idleReviewRadar.priorityCount, 0)
        XCTAssertEqual(idleReviewRadar.metadataPendingCount, 0)
        XCTAssertNil(idleReviewRadar.primaryReviewKind)
        let idleHandoffBrief = idleStore.missionRunSummary.macAgentHandoffBrief
        XCTAssertFalse(idleHandoffBrief.isReviewable)
        XCTAssertEqual(idleHandoffBrief.totalCount, 0)
        ...
        let preSendReviewRadar = preSendSummary.macAgentReviewRadar
        XCTAssertTrue(preSendReviewRadar.isReviewable)
        XCTAssertEqual(preSendReviewRadar.totalCount, 5)
        XCTAssertTrue(preSendReviewRadar.requiresHumanAction)
        XCTAssertEqual(preSendReviewRadar.primaryReviewKind, "approval")
        XCTAssertTrue(preSendReviewRadar.sectors.contains { $0.id == "human-handoff" && $0.reviewKind == "approval" && $0.humanActionCount > 0 })
        let focusedPreSendReviewRadar = preSendSummary.macAgentReviewRadar(focusedOn: "approval")
        XCTAssertEqual(focusedPreSendReviewRadar.focusedReviewKind, "approval")
        XCTAssertTrue(focusedPreSendReviewRadar.sectors.contains { $0.reviewKind == "approval" && $0.isFocused })
        let preSendHandoffBrief = preSendSummary.macAgentHandoffBrief
        XCTAssertTrue(preSendHandoffBrief.isReviewable)
        XCTAssertEqual(preSendHandoffBrief.totalCount, 5)
        XCTAssertTrue(preSendHandoffBrief.requiresHumanAction)
        XCTAssertEqual(preSendHandoffBrief.primaryReviewKind, "approval")
        XCTAssertTrue(preSendHandoffBrief.items.contains { $0.id == "human-confirmation" && $0.reviewKind == "approval" && $0.requiresHumanAction })
        ...
        let reviewRadar = summary.macAgentReviewRadar
        XCTAssertTrue(reviewRadar.isReviewable)
        XCTAssertEqual(reviewRadar.totalCount, 5)
        XCTAssertGreaterThan(reviewRadar.priorityCount, 0)
        XCTAssertTrue(reviewRadar.requiresHumanAction)
        XCTAssertTrue(reviewRadar.sectors.contains { $0.id == "safety-review" && $0.priorityCount > 0 })
        XCTAssertTrue(reviewRadar.sectors.contains { $0.id == "evidence-coverage" && $0.readyCount > 0 })
        XCTAssertTrue(reviewRadar.sectors.contains { $0.id == "loop-continuation" && $0.reviewKind == "agent-trace" })
        XCTAssertNotNil(reviewRadar.primaryReviewKind)
        let focusedReviewRadar = summary.macAgentReviewRadar(focusedOn: "delivery-safety")
        XCTAssertEqual(focusedReviewRadar.focusedReviewKind, "delivery-safety")
        XCTAssertEqual(focusedReviewRadar.focusedReviewTitle, "最终提交安全")
        XCTAssertTrue(focusedReviewRadar.sectors.contains { $0.reviewKind == "delivery-safety" && $0.isFocused })
        let handoffBrief = summary.macAgentHandoffBrief
        XCTAssertTrue(handoffBrief.isReviewable)
        XCTAssertEqual(handoffBrief.totalCount, 5)
        ...
        XCTAssertEqual(statusReviewRadar.focusedReviewKind, statusOnlyItem.reviewKind)
        XCTAssertEqual(statusReviewRadar.focusedReviewTitle, statusOnlyItem.title)
        XCTAssertTrue(statusReviewRadar.sectors.contains { $0.reviewKind == statusOnlyItem.reviewKind && $0.isFocused })
        ...
        XCTAssertNil(staleReviewRadar.focusedReviewKind)
        XCTAssertEqual(staleReviewRadar.totalCount, statusOnlySummary.macAgentReviewRadar.totalCount)
        ...
        let reviewRadarVisibleChunks = reviewRadar.sectors.flatMap {
            [
                $0.title,
                $0.status,
                $0.guidance,
        ...
        let handoffBriefVisibleChunks = handoffBrief.items.flatMap {
            [
                $0.title,
                $0.status,
                $0.guidance,
        ...
        let shellReviewRadar = shellStore.missionRunSummary.macAgentReviewRadar(focusedOn: "shell-safety")
        XCTAssertTrue(shellReviewRadar.isReviewable)
        XCTAssertTrue(shellReviewRadar.requiresHumanAction)
        XCTAssertEqual(shellReviewRadar.focusedReviewKind, "shell-safety")
        XCTAssertEqual(shellReviewRadar.focusedReviewTitle, "Shell 命令安全")
        XCTAssertTrue(shellReviewRadar.sectors.contains { $0.reviewKind == "shell-safety" && $0.isFocused })
        XCTAssertTrue(shellReviewRadar.sectors.contains { $0.id == "execution-state" && $0.blockedCount > 0 })
        let shellHandoffBrief = shellStore.missionRunSummary.macAgentHandoffBrief(focusedOn: "shell-safety")
        XCTAssertTrue(shellHandoffBrief.isReviewable)
        XCTAssertTrue(shellHandoffBrief.requiresHumanAction)
        XCTAssertEqual(shellHandoffBrief.focusedReviewKind, "shell-safety")
        ...
        let shellApprovalOutput: [String] = [
        ...
        let shellContinuationOutput: [String] = [
            ...
            shellReviewRadar.sectors.map { "\($0.title) \($0.status) \($0.guidance)" }.joined(separator: " "),
        ...
        let shellSensitiveOutput: String = [
            shellReviewOutput.joined(separator: " "),
            shellApprovalOutput.joined(separator: " "),
            shellDetailOutput.joined(separator: " "),
            shellContinuationOutput.joined(separator: " ")
        ].joined(separator: " ")
        XCTAssertFalse(shellSensitiveOutput.contains("stdout"))
    }
    ...
    func testMissionRunSummaryDerivesAgentTraceReview() throws {
        ...
        XCTAssertEqual(review.effectiveNextActionCount, 6)
        XCTAssertEqual(review.blockedNextActionCount, 0)
    ...
    func testMissionRunLoopContinuationCanBeReadyToContinue() throws {
        ...
        let unsafeArtifact = ClawGatewayArtifact(
            kind: .agentTrace,
            title: "agent-loop-unsafe-policy.json",
            reference: "file:///tmp/agent-loop-unsafe-policy.json",
            isRedacted: true,
            metadata: unsafeMetadata
        )
        let unsafeReview = try XCTUnwrap(ClawAgentTraceReviewSummary.latest(from: [unsafeArtifact]))
        XCTAssertNil(unsafeReview.nextActionPolicy)
        XCTAssertTrue(unsafeReview.requiresNextActionPolicyReview)
        var unsafeSummary = summary
        unsafeSummary.agentTraceReview = unsafeReview
        XCTAssertFalse(unsafeSummary.loopContinuationSummary.canContinueLoop)
        XCTAssertTrue(unsafeSummary.loopContinuationSummary.requiresHumanAction)

        var unauthorizedMetadata = artifact.metadata ?? [:]
        unauthorizedMetadata["selectedNextActionAllowedByEnvelope"] = "false"
        let unauthorizedArtifact = ClawGatewayArtifact(
            kind: .agentTrace,
            title: "agent-loop-unauthorized-selection.json",
            reference: "file:///tmp/agent-loop-unauthorized-selection.json",
            isRedacted: true,
            metadata: unauthorizedMetadata
        )
        let unauthorizedReview = try XCTUnwrap(ClawAgentTraceReviewSummary.latest(from: [unauthorizedArtifact]))
        XCTAssertTrue(unauthorizedReview.requiresNextActionPolicyReview)
        var unauthorizedSummary = summary
        ...
        let inconsistentArtifact = ClawGatewayArtifact(
            kind: .agentTrace,
            title: "agent-loop-inconsistent-counts.json",
            reference: "file:///tmp/agent-loop-inconsistent-counts.json",
            isRedacted: true,
            metadata: inconsistentMetadata
        )
        let inconsistentReview = try XCTUnwrap(ClawAgentTraceReviewSummary.latest(from: [inconsistentArtifact]))
        XCTAssertNil(inconsistentReview.requestedNextActionCount)
        XCTAssertTrue(inconsistentReview.requiresNextActionPolicyReview)
        var inconsistentSummary = summary
        inconsistentSummary.agentTraceReview = inconsistentReview
        XCTAssertFalse(inconsistentSummary.loopContinuationSummary.canContinueLoop)

        var contradictoryMetadata = artifact.metadata ?? [:]
        contradictoryMetadata["nextActionPolicyDiagnostic"] = "policy-blocked"
        let contradictoryArtifact = ClawGatewayArtifact(
            kind: .agentTrace,
            title: "agent-loop-contradictory-policy.json",
            reference: "file:///tmp/agent-loop-contradictory-policy.json",
            isRedacted: true,
            metadata: contradictoryMetadata
        )
        let contradictoryReview = try XCTUnwrap(ClawAgentTraceReviewSummary.latest(from: [contradictoryArtifact]))
        XCTAssertTrue(contradictoryReview.requiresNextActionPolicyReview)
        var contradictorySummary = summary
        ...
        let blockedArtifact = ClawGatewayArtifact(
            kind: .agentTrace,
            title: "agent-loop-policy-blocked.json",
        ...
        XCTAssertFalse(blockedSummary.loopContinuationSummary.canContinueLoop)
        XCTAssertTrue(blockedSummary.loopContinuationSummary.requiresHumanAction)
        XCTAssertTrue(continuation.canFocusAgentTrace)
    ...
    func testContinuationManageFilesEditorRejectsInvalidArguments() throws {
        let (store, _, _, _) = try makeManageFilesContinuationDraft()
        ...
        for (path, text, message) in invalidCases {
            XCTAssertFalse(store.updateContinuationFileArguments(writePath: path, writeText: text))
            XCTAssertEqual(store.continuationDraft?.state, .readyForInput)
            XCTAssertEqual(store.missionRunSummary.continuationDraft.fileArguments.validationMessage, message)
            XCTAssertFalse(store.missionRunSummary.continuationDraft.fileArguments.isValid)
            XCTAssertNil(store.continuationDraft?.childTaskID)
            XCTAssertEqual(store.clawMobileTasks.count, 1)
        }

        var unknownKeyArguments = ClawContinuationActionArguments.makeManageFilesArguments(
            writePath: "notes/report.txt",
            writeText: "body"
        )
        unknownKeyArguments["unexpectedKey"] = "private-marker"
        XCTAssertFalse(ClawContinuationActionArguments.validate(
            kind: .manageFiles,
            arguments: unknownKeyArguments
        ).isEmpty)
        let unknownKeyMessage = ClawContinuationActionArguments.validationMessage(
            kind: .manageFiles,
            arguments: unknownKeyArguments
        )
        XCTAssertEqual(unknownKeyMessage, "文件参数包含不支持的字段。")
    ...
    func testContinuationManageFilesEditorKeepsApprovalGateAfterValidArguments() throws {
        ...
        let parentFocus = ClawMissionRunReviewFocus(scopeID: parentScope, reviewKind: "agent-trace")
        let sourceDigest = draft.sourceDecisionDigest
        let receiptHandle = draft.receiptHandle

        XCTAssertTrue(store.missionRunSummary.continuationDraft.fileArguments.isVisible)
        XCTAssertTrue(store.missionRunSummary.continuationDraft.fileArguments.isEditable)
        ...
        XCTAssertTrue(store.updateContinuationFileArguments(
            writePath: "notes/report.txt",
        ...
        XCTAssertNil(store.continuationDraft?.childTaskID)
        XCTAssertEqual(store.missionRunSummary.continuationDraft.fileArguments.writePath, "notes/report.txt")
        XCTAssertEqual(store.missionRunSummary.continuationDraft.fileArguments.writeText, "approved body")
        XCTAssertTrue(store.missionRunSummary.continuationDraft.fileArguments.isValid)
        XCTAssertTrue(store.missionRunSummary.continuationDraft.fileArguments.isEditable)
        XCTAssertEqual(store.missionRunSummary.taskID, parentTask.id)
        XCTAssertEqual(store.missionRunSummary.sessionID, parentSession.id)
        XCTAssertEqual(store.missionRunSummary.activeReviewFocus(from: parentFocus), "agent-trace")
    ...
    func testAgentTraceReviewRedactsSensitiveMetadata() throws {
        ...
        let visibleText = [
            ...
            review.nextActionPolicy ?? "",
            review.nextActionPolicyDiagnostic ?? "",
        ...
        XCTAssertNil(review.requestedNextActionCount)
        XCTAssertNil(review.effectiveNextActionCount)
        XCTAssertNil(review.blockedNextActionCount)
        XCTAssertNil(review.selectedNextActionAllowedByEnvelope)
        XCTAssertTrue(review.requiresNextActionPolicyReview)
        XCTAssertNil(review.handoffStatus)
        XCTAssertFalse(visibleText.contains("Authorization"))
        XCTAssertFalse(visibleText.contains("Bearer"))
        XCTAssertFalse(visibleText.contains("raw-token"))
    ...
    func testGatewayPairingDiagnosticsSeparatesConfiguredFromConfirmed() async throws {
        let configuredStore = ClawStore(autoScanLocalArtifacts: false)

        let empty = configuredStore.gatewayPairingDiagnosticsSummary
        XCTAssertEqual(empty.state, .unavailable)
        XCTAssertFalse(empty.isVisible)

        configuredStore.gatewayDispatchMode = .liveGateway
        configuredStore.setGateway(url: "ws://127.0.0.1:18789", token: "paired-secret")
        configuredStore.phoneAgentCommand = "打开浏览器搜索资料"
        configuredStore.generatePhoneAgentPlan()
        configuredStore.queueClawMobileTaskFromCurrentPlan()
        configuredStore.approveLatestClawMobileTask()
        configuredStore.sendLatestClawMobileTask()

        let configured = configuredStore.gatewayPairingDiagnosticsSummary
        ...
        XCTAssertFalse(configuredText.contains(configuredStore.clawMobileTasks[0].id.uuidString))

        let resume = configuredStore.gatewayResumeIntentPresentationSummary
        let requestBefore = configuredStore.lastGatewayLiveRequest
        XCTAssertFalse(resume.canPrepare)
        XCTAssertFalse(configuredStore.prepareExplicitResumeIntent(for: resume))
        XCTAssertEqual(configuredStore.lastGatewayLiveRequest, requestBefore)

        let confirmedStore = ClawStore(autoScanLocalArtifacts: false)
        confirmedStore.setGateway(url: "ws://127.0.0.1:18789", token: "paired-secret")
        confirmedStore.phoneAgentCommand = "打开浏览器搜索资料"
        confirmedStore.generatePhoneAgentPlan()
        confirmedStore.queueClawMobileTaskFromCurrentPlan()
        confirmedStore.approveLatestClawMobileTask()

        await confirmedStore.sendLatestClawMobileTaskOverLiveGateway(transport: MockClawGatewayTransport())
        XCTAssertEqual(confirmedStore.gatewayPairingDiagnosticsSummary.state, .completed)
        XCTAssertNotEqual(confirmedStore.gatewayPairingDiagnosticsSummary.state, .fallbackSimulated)
    }
