...
- `Tools/claw-gateway-server.mjs`
  ...
  - 在 session 开始后写入 `gateway-capability-snapshot.json` `auditLog` artifact，审计 workspace、platform、token 短指纹、allowlist 和 real/dry-run/disabled/unavailable 状态。
...
4. 立即写入 session 级 `gateway-capability-snapshot.json` `auditLog` artifact，并推一个无 action 绑定的 `artifactStored`。
...
- `gateway-capability-snapshot.json` 只能记录安全策略摘要和短 token 指纹，不能写入 raw token、Authorization header、自然语言 instruction、`toolArguments`、网页正文、命令输出、截图内容或草稿正文，也不能作为执行计划来源。
