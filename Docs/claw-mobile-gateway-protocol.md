...
v0.21 起，`controlBrowser` 的 `browserTrace` 和 `browser-control-*.json` artifact event 会附带 metadata-only 的浏览器控制计划复核摘要。建议 metadata 键包括 `browserReview=controlPlan`、`mode`、`actionKind=controlBrowser`、`browserControlPolicy`、`policyDiagnostic`、`retryableReason`、`browserControlRequested`、`openInBrowser`、`openAttempted`、`targetURLPresent`、`searchQueryPresent`、`localHTMLInput`、`networkFetchAttempted`、`networkFetchSucceeded`、`networkBlocked`、`networkPolicyDiagnostic`、`redirectPolicyChecked`、`redirectCount`、`redirectBlocked`、`redirectLimitExceeded`、`appAllowlistEnforced`、`hostAllowlistEnforced`、`appPolicyChecked`、`hostPolicyChecked`、`executed`、`timedOut`、`resultStatus` 和 `safetyFlags`。v0.48 起，`policyDiagnostic` 和 `retryableReason` 只能使用固定枚举，用于表达桌面浏览器打开策略。v0.61 起，独立的 `networkPolicyDiagnostic` 只能使用 `not-requested`、`fetch-succeeded`、`initial-host-blocked`、`initial-protocol-blocked`、`initial-credentials-blocked`、`redirect-host-blocked`、`redirect-protocol-blocked`、`redirect-credentials-blocked`、`redirect-location-invalid`、`redirect-limit-exceeded`、`fetch-timeout`、`http-error` 或 `network-error`；`redirectCount` 只接受 0 到 5。手机端可显示 network fetch/blocked、逐跳策略检查和固定诊断；不得读取 `file://` payload，不得展示 raw URL、host、Location、redirect chain、search query、HTML/page text、headers/cookie、browser app name、allowlist 值、token 或 `toolArguments`。metadata 缺失时必须降级为“metadata 待同步”。该摘要不新增 schema 字段、action kind、artifact kind 或 event kind，也不扩大 Gateway 权限。

v0.22 起，`manageFiles` 的既有 `fileDiff` artifact event 以及路径阻断/写入失败审计 artifact event 会附带 metadata-only 的文件变更安全复核摘要；v0.52 起增加固定 `filePolicyDiagnostic`、`fileRetryableReason`、`policyChecked`、`workspacePolicyChecked` 和 `pathPolicyChecked`。建议 metadata 键包括 `fileChangeReview=workspaceWrite`、`mode`、`actionKind=manageFiles`、`workspacePolicy=session-workspace-only`、`workspaceScoped`、`pathEscapeBlocked`、`writeAttempted`、`writeSucceeded`、`createdFileCount`、`modifiedFileCount`、`deletedFileCount`、`requestedPathPresent`、`writeTextPresent`、`rawPathOmitted`、`contentOmitted`、`diffOmitted`、`resultStatus` 和 `safetyFlags`。手机端可显示 workspace policy、写入是否尝试/成功、路径逃逸是否阻断、变更计数和 path/content/diff 省略状态；不得读取 `file://` payload，不得展示 raw path、workspace/sessionWorkspace、文件名/目录名、文件内容、diff hunk、patch、stdout/stderr、raw token、Authorization/header、cookie 或 `toolArguments`。metadata 缺失时必须降级为“metadata 待同步”。该摘要不新增 schema 字段、action kind、artifact kind 或 event kind，也不扩大 Gateway 权限。

v0.23 起，`runShellCommand` 的既有 `commandOutput` artifact event 会附带 metadata-only 的 Shell 命令安全复核摘要。建议 metadata 键包括 `shellReview=commandSafety`、`mode`、`actionKind=runShellCommand`、`shellPolicy`、`shellPolicyDiagnostic`、`shellRetryableReason`、`policyChecked`、`binaryAllowlistChecked`、`structuredCommandChecked`、`structuredCommandPresent`、`commandParsed`、`allowlistConfigured`、`allowlistMatched`、`executionAttempted`、`executed`、`timedOut`、`exitCodePresent`、`exitCodeZero`、`stdoutPresent`、`stderrPresent`、`commandOmitted`、`stdoutOmitted`、`stderrOmitted`、`cwdOmitted`、`resultStatus` 和 `safetyFlags`。v0.51 起，`shellPolicyDiagnostic` 固定为 `not-requested`、`missing-structured-command`、`command-parse-failed`、`dry-run`、`allowlist-blocked`、`execution-attempted` 或 `execution-failed`，`shellRetryableReason` 固定为 `none`、`provide-structured-command`、`fix-command-parse`、`enable-shell`、`configure-shell-allowlist`、`allow-shell-binary`、`automation-failed` 或 `review-execution`。v0.58 起，Shell disabled/dry-run 在 binary allowlist 查询前短路时固定输出 `binaryAllowlistChecked=false`；只有 Shell enabled 且实际进入 binary allowlist 判断时才为 `true`。v0.59 起，binary allowlist 只接受不含 `/` 或 `\\` 的裸 executable token；同名路径会按 `allowlist-blocked` 阻断，保持 `allowlistMatched=false`、`executionAttempted=false`，事件和 metadata 不回显原始路径。v0.60 起新增 `invalid-structured-command-source` diagnostic 和 `invalid-command-source-blocked` safety flag；action 顶层 `shellCommand` / `commandLine` 是 `runShellCommand` 的明确禁止字段，任一存在时，即使同时提供合法嵌套命令，也会在 parse、binary allowlist 与执行前 fail closed。手机端可显示结构化命令是否存在、解析状态、Shell policy、策略诊断、重试原因、policy/binary/structured 检查状态、allowlist 是否命中、是否尝试/完成执行、exit code 是否可复核、stdout/stderr 是否存在以及 command/stdout/stderr/cwd 省略状态；不得读取 `file://` payload，不得展示 raw command、binary/args、cwd、workspace/session path、stdout/stderr 内容、raw token、Authorization/header、cookie、自然语言 instruction 或 `toolArguments`。metadata 缺失时必须降级为“metadata 待同步”。该摘要不新增 schema 字段、action kind、artifact kind 或 event kind，也不扩大 Gateway Shell 权限。
...
v0.69 起，regular Dock 在上述 metadata-only 详情之前就地复用 `primaryActionTitle`、`primaryActionIcon`、`primaryActionKind` 和 `isPrimaryActionEnabled`，并通过共享 presentation dispatcher 调用既有 `ClawStore` 主动作闸门。regular 左栏不再显示第二个可执行主动作，compact iPhone 保持原单栏入口。dispatcher 接收按钮渲染时的 summary，并在调用 Store 前同时校验 command、task/session/sessionTask、mission scope、phase、primary title/icon/kind 和 enabled 状态均仍与当前 summary 匹配；旧 summary、旧 scope、stale action 或 disabled action 均 fail closed。`.waitForGateway` 与 `.inspectBlocked` 不执行任何 Store 方法。该控件只触发现有启动、手机审批后继续或复核后重试流程，不新增 envelope、event、artifact 或 Gateway 能力，不改变 continuation receipt、freeze/send、workspace、allowlist 或 live preflight 合同。app/test 的 `TARGETED_DEVICE_FAMILY=1,2` 只扩大 iPad 工程覆盖；这不代表存在原生 macOS target。
...
- `Tools/claw-gateway-server.mjs`
  ...
  - Shell 只读取 `toolArguments.shellCommand`；顶层 `shellCommand` / `commandLine` 任一存在都会 action-bound fail closed，不回退或选择其中命令。默认 dry-run，必须设置 `CLAW_ALLOW_SHELL=1`，且 command token 是不含 `/` 或 `\\` 的裸 executable 名并精确命中 `CLAW_SHELL_ALLOWLIST`，才会真执行。同名路径不会因 basename 命中而获准。v0.23 起，`commandOutput` artifact event 会附 Shell Command Safety metadata，只写结构化命令 presence、parse/policy/allowlist/执行/exit code/stdout/stderr presence 和 omission flags，不写 raw command、cwd/path、stdout/stderr 内容或 `toolArguments`。
  - `controlBrowser` 可处理 `toolArguments.html`，也可在 `CLAW_ALLOW_BROWSER_NETWORK=1` 且 host allowlist 通过时抓取 URL；v0.61 起 Gateway fetch 手动处理最多 5 次重定向，每跳请求前重新检查 HTTP(S)、URL credentials 和精确 hostname allowlist，整条链路共享 12 秒超时。策略失败写 action-bound 脱敏 Browser artifact 并只失败当前 action。默认只写入桌面浏览器打开/搜索计划；设置 `CLAW_ALLOW_BROWSER_CONTROL=1`、`CLAW_BROWSER_APP_ALLOWLIST` 和 `CLAW_BROWSER_HOST_ALLOWLIST` 后，可在 macOS 上打开允许的浏览器。逐跳保证只覆盖 Gateway fetch，AppleScript 打开后的桌面浏览器导航仍由浏览器处理，不在本轮控制范围内。
