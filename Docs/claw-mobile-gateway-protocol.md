...
`URLSessionClawGatewayTransport` 会在客户端 WebSocket 打开并发送 envelope 后生成一个本地 `.gatewayConnected` 进度事件，表示“客户端已打开 WebSocket 并等待桌面端事件”。桌面 Gateway 随后也可以推送自己的 `.gatewayConnected`；手机端 health summary 会统计重复连接事件并把非终态 live 事件保持为 `streaming`。这属于兼容展示语义，不新增 event kind，也不代表自动重连、心跳或后台保活已经实现。
