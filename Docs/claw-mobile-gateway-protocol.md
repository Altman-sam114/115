...
- `artifacts`: 网关回传的截图、可访问性树、命令输出、文件 diff、浏览轨迹、消息草稿和审计日志引用。
...
v0.2 增加的 Mission Run 任务回合摘要只存在于手机端展示层。`ClawMissionRunSummary` 从 `ClawAutonomousLoopState`、最新 `ClawMobileTask` 和最新 `ClawGatewaySession` 派生，用于在电脑接管首屏展示当前目标、阶段、回合进度、下一步主动作、风险分、审批点、阻断数、artifact kind 摘要和成功/失败/可重试计数。

这不是协议字段：Mission Run 不写入 `ClawMobileEnvelope`，不改变 `claw.computer.control.v1` schema，不新增 action kind、artifact kind、Gateway event kind，也不扩大桌面 Gateway 的执行权限。桌面端仍只接收结构化 `task.actions[].toolArguments`，手机端仍只负责计划、审批、发送 envelope 和查看事件。
...
```json
{
  ...
  "artifacts": [
    {
      "kind": "browserTrace",
      "title": "browser-trace-2.json",
      "reference": "browserTrace://browser-trace-2.json",
      "isRedacted": false
    }
  ],
  "isRetryable": false,
...
4. 对每个 action 推 `actionStarted`。
5. 如有截图、命令输出、文件 diff、浏览轨迹或草稿，推 `artifactStored`。
6. 推 `actionCompleted`、`actionFailed`、`approvalRequested` 或 `actionSkipped`。
7. 最后推 `sessionCompleted`。
