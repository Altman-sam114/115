...
当前版本不下载模型权重，模型保持占位状态。App 已完成 UI、数据流、本地 artifact 导入/扫描/校验、电脑接管规划器、Claw Gateway envelope、事件流 reducer、Mission Run 任务回合面板、Mission Run Mac Agent Control Snapshot 控制态势快照、Mission Run Operator Strip、Mission Run Loop 继续态势、Mission Run Mac Agent Readiness Board 就绪看板、Mission Run Mac Gateway Action Preflight Matrix 动作预检矩阵、Mission Run Mac Agent Evidence Coverage Map 证据覆盖图、Mission Run Mac Agent Next Step Deck 下一步候选卡组、Mission Run Mac Agent Run Timeline 执行时间线、Mission Run Mac Agent Continuation Gate 继续闸门、Mission Run Mac Agent Review Radar 复核雷达、Mission Run Mac Agent Handoff Brief 人工交接简报、Mission Run Focus Context 聚焦上下文、Mission Run Review Detail Dock、Mission Run Review Trail 复核路径、Mission Run Approval Queue 审批队列、Mission Run Payload Safety Ledger 载荷安全账本、Mission Run Artifact 证据索引、Mission Run 复核优先队列、复核聚焦模式、复核态势摘要与下一步复核行动、Artifact metadata 详情复核、文件变更安全复核、提取完整性复核、草稿/最终提交安全复核、AgentTrace handoff 状态、mac 证据质量分层和复核摘要、Gateway 能力复核摘要、Accessibility artifact 复核摘要、Gateway Replay Guard 复核摘要、专用 metadata 复核统一脱敏、Live Gateway 连接健康摘要、有界重连与 ping 可观测性、Gateway 进程内 task replay guard、iPad 多栏复核工作台、Gateway 能力快照审计、macOS Accessibility 观察摘要、WebSocket transport 边界、Shortcuts 入口和 smoke 测试。
...
- 手机端：输入任务、生成计划、用 Mission Run 面板展示当前阶段/下一步/风险/证据、Control Snapshot 控制态势快照、Operator Strip、Loop 继续态势、Mac Agent Readiness Board 就绪看板、Action Preflight Matrix 动作预检矩阵、Evidence Coverage Map 证据覆盖图、Mac Agent Next Step Deck 下一步候选卡组、Mac Agent Run Timeline 执行时间线、Mac Agent Continuation Gate 继续闸门、Mac Agent Review Radar 复核雷达、Mac Agent Handoff Brief 人工交接简报、Focus Context 聚焦上下文、Review Trail 复核路径、Approval Queue 审批队列、Payload Safety Ledger 载荷安全账本、Artifact 证据索引、复核态势摘要、下一步人工复核行动和按风险/可行动性排序的复核优先队列，可聚焦单个队列项查看对应详细复核，审批高风险动作、查看 envelope、Live Gateway 连接健康、Artifact metadata、文件变更安全、提取完整性、草稿/最终提交安全、Gateway 能力、Accessibility、Replay Guard 和 AgentTrace handoff 审计摘要；iPad/宽屏下把命令、Mission Run、计划、Gateway 会话、事件、权限和日志分栏复核，并在右侧顶部同步显示当前 Mission 复核详情 Dock、控制态势快照、Mac Agent 就绪看板、动作预检矩阵、证据覆盖图、下一步候选卡组、执行时间线、继续闸门、复核雷达、交接简报、复核路径、载荷安全账本和审批队列。
...
v0.40 起，Mission Run Mac Agent Evidence Coverage Map 是手机端 presentation-layer 证据覆盖图：它从 Action Preflight Matrix、Artifact Evidence Index、Payload Safety Ledger 和复核优先队列派生每个复核域的 action 支撑、artifact 证据、metadata、payload 边界和人工复核状态。Coverage Map 在 compact Mission Run 和 iPad/mac 右侧 Dock 展示；row 只改变本地聚焦，不执行 Gateway 动作、不审批、不发送、不重试、不自动继续，不写入 envelope，不读取或展示 action `toolArguments` 原文、artifact `reference`、Gateway `file://` payload、文件内容、命令输出、diff、网页正文或草稿正文。

v0.41 起，Mission Run Mac Agent Next Step Deck 是手机端 presentation-layer 下一步候选卡组：它从 review readiness、next review action、Loop 继续态势、Mac Agent Readiness Board、Action Preflight Matrix、Evidence Coverage Map 和 Approval Queue 派生人工确认、证据补齐、失败复核、Loop 下一步和抽查复核候选。Deck 在 compact Mission Run 的 Evidence Coverage Map 后展示，也在 iPad/mac 右侧 Dock 的 Evidence Coverage Map 后复用；row 只改变本地聚焦，不执行 Gateway 动作、不审批、不发送、不重试、不自动继续，不写入 envelope，不回传 Gateway，不读取 action `toolArguments` 原文、artifact `reference`、Gateway `file://` payload、文件内容、命令输出、diff、网页正文或草稿正文。

v0.42 起，Mission Run Mac Agent Run Timeline 是手机端 presentation-layer 执行时间线：它从 Action Preflight Matrix、Evidence Coverage Map、Approval Queue 和 Review Readiness 派生 action 顺序、Gateway 结果、证据同步、metadata 缺口和人工交接状态。Timeline 在 compact Mission Run 的 Next Step Deck 后展示，也在 iPad/mac 右侧 Dock 的 Next Step Deck 后、Review Trail 前复用；row 只改变本地聚焦，不执行 Gateway 动作、不审批、不发送、不重试、不自动继续，不写入 envelope，不回传 Gateway，不读取 action `toolArguments` 原文、artifact `reference`、Gateway `file://` payload、`auditTrail` 原文、文件内容、命令输出、stdout/stderr、diff、网页正文、草稿正文、token 或 header。

v0.43 起，Mission Run Mac Agent Continuation Gate 是手机端 presentation-layer 继续闸门：它从 Next Step Deck、Run Timeline、Approval Queue、Review Readiness、Loop 继续态势和 Mac Agent Readiness Board 派生阻断、可重试、人工确认、metadata/证据缺口、Loop 可继续和抽查入口。Gate 在 compact Mission Run 的 Run Timeline 后展示，也在 iPad/mac 右侧 Dock 的 Run Timeline 后、Review Trail 前复用；row 只改变本地聚焦，不执行 Gateway 动作、不审批、不发送、不重试、不自动继续，不写入 envelope，不回传 Gateway，不读取 action `toolArguments` 原文、artifact `reference`、Gateway `file://` payload、`auditTrail` 原文、文件内容、命令输出、stdout/stderr、diff、网页正文、草稿正文、token、header、cookie 或 secret。

v0.44 起，Mission Run Mac Agent Review Radar 是手机端 presentation-layer 复核雷达：它从 Review Readiness、Evidence Coverage Map、Action Preflight Matrix、Approval Queue、Continuation Gate、Loop 继续态势和 Mac Agent Readiness Board 派生安全复核、证据覆盖、执行状态、人工交接和 Loop 继续五个固定 sector。Radar 在 compact Mission Run 的 Continuation Gate 后展示，也在 iPad/mac 右侧 Dock 的 Continuation Gate 后、Review Trail 前复用；row 只改变本地聚焦，不执行 Gateway 动作、不审批、不发送、不重试、不自动继续，不写入 envelope，不回传 Gateway，不读取 action `toolArguments` 原文、artifact `reference`、Gateway `file://` payload、`auditTrail` 原文、文件内容、命令输出、stdout/stderr、diff、网页正文、草稿正文、token、header、cookie 或 secret。

v0.45 起，Mission Run Mac Agent Handoff Brief 是手机端 presentation-layer 人工交接简报：它从 Review Radar、Continuation Gate、Next Step Deck、Run Timeline、Approval Queue、Focus Context 和 Loop 继续态势派生阻断/可重试、人工确认、metadata/证据缺口、Loop 下一轮和抽查复核五个固定简报项。Brief 在 compact Mission Run 的 Review Radar 后展示，也在 iPad/mac 右侧 Dock 的 Review Radar 后、Review Trail 前复用；row 只改变本地聚焦，不执行 Gateway 动作、不审批、不发送、不重试、不自动继续，不写入 envelope，不回传 Gateway，不读取 action `toolArguments` 原文、artifact `reference`、Gateway `file://` payload、`auditTrail` 原文、文件内容、命令输出、stdout/stderr、diff、网页正文、草稿正文、token、header、cookie 或 secret。

v0.46 起，Mission Run Mac Agent Control Snapshot 是手机端 presentation-layer 控制态势快照：它从 Handoff Brief、Continuation Gate、Review Radar、Approval Queue、Loop 继续态势、Operator Strip 和 Focus Context 派生当前控制状态、阻断/人工/metadata/可重试/显式下一轮信号和一个本地聚焦入口。Snapshot 在 compact Mission Run 的 Operator Strip 前展示，也在 iPad/mac 右侧 Dock 顶部复用；按钮只改变本地聚焦，不执行 Gateway 动作、不审批、不发送、不重试、不自动继续，不写入 envelope，不回传 Gateway，不读取 action `toolArguments` 原文、artifact `reference`、Gateway `file://` payload、`auditTrail` 原文、文件内容、命令输出、stdout/stderr、diff、网页正文、草稿正文、token、header、cookie 或 secret。
...
- 2026-07-07：新增 v0.46 Mac Agent Control Snapshot 控制态势快照。手机端从 Handoff Brief、Continuation Gate、Review Radar、Approval Queue、Loop 继续态势、Operator Strip 和 Focus Context 派生顶层控制状态，区分 idle、聚焦、阻断、等待人工、可重试、metadata 待同步、可显式继续和可抽查；compact Mission Run 和 iPad/mac 右侧 Dock 复用同一快照。Snapshot 只改变本地聚焦，不读取或展示 `toolArguments` 原文、Gateway `file://` payload、artifact `reference`、`auditTrail` 原文、文件内容、命令输出、stdout/stderr、diff、网页正文、草稿正文、token、header、cookie 或 secret，不执行 Gateway 动作、不审批、不发送、不重试、不自动继续。本轮不新增 schema/event/action/artifact kind，不扩大 Gateway 权限。
- 2026-07-07：新增 v0.45 Mac Agent Handoff Brief 人工交接简报。手机端从 Review Radar、Continuation Gate、Next Step Deck、Run Timeline、Approval Queue、Focus Context 和 Loop 继续态势派生阻断/可重试、人工确认、metadata/证据缺口、Loop 下一轮和抽查复核五个固定 item；compact Mission Run 和 iPad/mac 右侧 Dock 复用同一简报。Brief 只改变本地聚焦，不读取或展示 `toolArguments` 原文、Gateway `file://` payload、artifact `reference`、`auditTrail` 原文、文件内容、命令输出、stdout/stderr、diff、网页正文、草稿正文、token、header、cookie 或 secret，不执行 Gateway 动作、不审批、不发送、不重试、不自动继续。本轮不新增 schema/event/action/artifact kind，不扩大 Gateway 权限。
- 2026-07-07：新增 v0.44 Mac Agent Review Radar 复核雷达。手机端从 Review Readiness、Evidence Coverage Map、Action Preflight Matrix、Approval Queue、Continuation Gate、Loop 继续态势和 Mac Agent Readiness Board 派生安全复核、证据覆盖、执行状态、人工交接和 Loop 继续五个固定 sector；compact Mission Run 和 iPad/mac 右侧 Dock 复用同一雷达。Radar 只改变本地聚焦，不读取或展示 `toolArguments` 原文、Gateway `file://` payload、artifact `reference`、`auditTrail` 原文、文件内容、命令输出、stdout/stderr、diff、网页正文、草稿正文、token、header、cookie 或 secret，不执行 Gateway 动作、不审批、不发送、不重试、不自动继续。本轮不新增 schema/event/action/artifact kind，不扩大 Gateway 权限。
- 2026-07-07：新增 v0.43 Mac Agent Continuation Gate 继续闸门。手机端从 Next Step Deck、Run Timeline、Approval Queue、Review Readiness、Loop 继续态势和 Mac Agent Readiness Board 派生继续闸门，压缩展示阻断、可重试、人工确认、metadata/证据缺口、Loop 可继续和抽查入口；compact Mission Run 和 iPad/mac 右侧 Dock 复用同一闸门。Gate 只改变本地聚焦，不读取或展示 `toolArguments` 原文、Gateway `file://` payload、artifact `reference`、`auditTrail` 原文、文件内容、命令输出、stdout/stderr、diff、网页正文、草稿正文、token、header、cookie 或 secret，不执行 Gateway 动作、不审批、不发送、不重试、不自动继续。本轮不新增 schema/event/action/artifact kind，不扩大 Gateway 权限。
- 2026-07-07：新增 v0.42 Mac Agent Run Timeline 执行时间线。手机端从动作预检、证据覆盖、审批队列和复核态势派生 action 顺序、Gateway 结果、证据同步、metadata 缺口和人工交接状态；compact Mission Run 和 iPad/mac 右侧 Dock 复用同一时间线。Timeline 只改变本地聚焦，不读取或展示 `toolArguments` 原文、Gateway `file://` payload、artifact `reference`、`auditTrail` 原文、文件内容、命令输出、stdout/stderr、diff、网页正文、草稿正文、token 或 header，不执行 Gateway 动作、不审批、不发送、不重试、不自动继续。本轮不新增 schema/event/action/artifact kind，不扩大 Gateway 权限。
- 2026-07-07：新增 v0.41 Mac Agent Next Step Deck 下一步候选卡组。手机端从复核态势、下一步复核行动、Loop 继续态势、Mac Agent 就绪、动作预检、证据覆盖和审批队列派生人工确认、证据补齐、失败复核、Loop 下一步和抽查复核候选；compact Mission Run 和 iPad/mac 右侧 Dock 复用同一卡组。Deck 只改变本地聚焦，不读取或展示 `toolArguments` 原文、Gateway `file://` payload、artifact `reference`、文件内容、命令输出、diff、网页正文或草稿正文，不执行 Gateway 动作、不审批、不发送、不重试、不自动继续。本轮不新增 schema/event/action/artifact kind，不扩大 Gateway 权限。
- 2026-07-07：新增 v0.40 Mac Agent Evidence Coverage Map 证据覆盖图。手机端从 Action Preflight Matrix、Artifact Evidence Index、Payload Safety Ledger 和复核优先队列派生每个复核域的 action 支撑、artifact 证据、metadata、payload 边界和人工复核状态；compact Mission Run 和 iPad/mac 右侧 Dock 复用同一覆盖图。Coverage Map 只改变本地聚焦，不读取或展示 `toolArguments` 原文、Gateway `file://` payload、artifact `reference`、文件内容、命令输出、diff、网页正文或草稿正文，不执行 Gateway 动作、不审批、不发送、不重试、不自动继续。本轮不新增 schema/event/action/artifact kind，不扩大 Gateway 权限。
