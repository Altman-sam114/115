...
function browserControlReviewMetadata(action, browserInput, plan, details = {}) {
  ...
  return compactMetadata({
    ...
    policyDiagnostic: details.policyDiagnostic,
    retryableReason: details.retryableReason,
    browserControlRequested,
    openInBrowser: browserControlRequested,
    openAttempted: Boolean(details.openAttempted),
    targetURLPresent,
    searchQueryPresent,
    ...
    hostPolicyChecked: Boolean(details.hostPolicyChecked),
...
async function browserControlArtifact(action, index, browserInput, config) {
  ...
  if (process.platform !== "darwin") {
    ...
    const metadata = browserControlReviewMetadata(action, browserInput, plan, {
      mode: payload.mode,
      browserControlPolicy: payload.browserControlPolicy,
      policyDiagnostic: "platform-unavailable",
  ...
  if (!allowlistContains(config.browserAppAllowlist, plan.browserApp)) {
    ...
    const metadata = browserControlReviewMetadata(action, browserInput, plan, {
      mode: payload.mode,
      browserControlPolicy: payload.browserControlPolicy,
      policyDiagnostic: "app-blocked",
      retryableReason: "allow-browser-app",
      openAttempted: false,
  ...
  if (plan.targetURL) {
    ...
    if (!config.browserHostAllowlist.has(url.hostname)) {
      ...
      const metadata = browserControlReviewMetadata(action, browserInput, plan, {
        mode: payload.mode,
        browserControlPolicy: payload.browserControlPolicy,
        policyDiagnostic: "host-blocked",
        retryableReason: "allow-browser-host",
        openAttempted: false,
        appAllowlistEnforced: true,
  ...
  const metadata = browserControlReviewMetadata(action, browserInput, plan, {
    mode: payload.mode,
    browserControlPolicy: payload.browserControlPolicy,
    policyDiagnostic: payload.executed ? "opened" : "automation-failed",
    retryableReason: payload.executed ? "none" : "automation-failed",
    openAttempted: true,
    appAllowlistEnforced: true,
    hostAllowlistEnforced: Boolean(plan.targetURL),
    appPolicyChecked: true,
    hostPolicyChecked: Boolean(plan.targetURL),
