...
- 修改 Mission Run presentation summary、复核优先队列或 SwiftUI 展示派生模型。
...
- Swift logic smoke 必须覆盖 Mission Run 复核优先队列的空状态、排序稳定、高风险复核项和敏感字符串不外显；XCTest 也应覆盖同类 presentation-layer 派生。
